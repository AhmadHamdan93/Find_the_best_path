...
def initialize_population():
    return [np.random.randint(number_range[0], number_range[1] + 1, array_size) for _ in range(population_size)]
...
def mutate(child):
    for i in range(array_size):
        if np.random.random() < mutation_rate:
            child[i] = np.random.randint(number_range[0], number_range[1] + 1)
    return child


# Genetic algorithm
def genetic_algorithm_2():
